...
class ViewController: UITableViewController {
    var prime = PrimeNumbers.generate(upperBound: 103, from: 2)
    var scrollIndex = 1
    ...
    override func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        ...
        if indexPath.row == prime.count - 2 {
            scrollIndex += 1
            let lastPrimeNum = prime.last?.last
            let nextPrimeNumArray = PrimeNumbers.findPrimeNumberlist(fromNumber: lastPrimeNum!, toNumber: scrollIndex * 103)
